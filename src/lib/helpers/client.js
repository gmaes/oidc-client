  ...
  if (this[`${endpoint}_endpoint_auth_method`] === "client_secret_jwt") {
    if (!alg) {
      ...
      alg =
        ...
        supported.find((signAlg) => /^HS(?:256|384|512)/.test(signAlg));
    }
...
async function authFor(endpoint, { clientAssertionPayload } = {}) {
  ...
  switch (authMethod) {
    ...
    case "client_secret_post":
      ...
      return {
        form: { client_id: this.client_id, client_secret: this.client_secret },
      };
    ...
    case "client_secret_jwt": {
      ...
      const audience = [
        ...new Set(
          [this.issuer.issuer, this.issuer.token_endpoint].filter(Boolean)
        ),
      ];
    ...
    case "client_secret_basic": {
      ...
      const encoded = `${formUrlEncode(this.client_id)}:${formUrlEncode(
        this.client_secret
      )}`;
    ...
    default: {
      throw new TypeError(
        `missing, or unsupported, ${endpoint}_endpoint_auth_method`
      );
    }
  }
}
...
async function authenticatedPost(
  ...
  console.log("authenticated post", { auth });
