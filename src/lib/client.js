...
class BaseClient {
  ...
  constructor(issuer, aadIssValidation, metadata = {}, jwks, options) {
    ...
    if (options != null && options.additionalAuthorizedParties) {
      this.#additionalAuthorizedParties = clone(
        options.additionalAuthorizedParties
      );
    }
...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  endSessionUrl(params = {}) {
    ...
    const { post_logout_redirect_uri = length === 1 ? postLogout : undefined } =
      params;
  ...
  async decryptJWE(jwe, expectedAlg, expectedEnc = "A128CBC-HS256") {
    ...
    if (expectedAlg.match(/^(?:RSA|ECDH)/)) {
      ...
      plaintext = await jose
        .compactDecrypt(
          jwe,
          this.secretForAlg(expectedAlg === "dir" ? expectedEnc : expectedAlg)
        )
        .then(getPlaintext, () => {});
    }
  ...
  async userinfo(
    ...
    const jwt = !!(
      this.userinfo_signed_response_alg || this.userinfo_encrypted_response_alg
    );
  ...
  async requestObject(
    requestObject = {},
    {
      ...
      encrypt: {
        ...
        enc: eContentEncryption = this.request_object_encryption_enc ||
          "A128CBC-HS256",
      } = {},
    } = {}
  ) {
